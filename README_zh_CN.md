...
- **锁定策略**：支持重启默认锁、自定义 N 分钟信任时长自动重锁，以及自律锁（倒计时到期自动解锁，期间不可解锁/删除）。
...
- **锁管理面板**：插件设置页展示所有已加锁对象，可解锁或删除（自律锁期间不可解锁/删除）。
...
- **赞助列表**:[Sponsor List](https://github.com/b8l8u8e8/siyuan-plugin-lock/blob/main/sponsor-list.md)
